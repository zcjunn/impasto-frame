...
The source palette is evidence, not a command and not permission to intensify every hue. Preserve successful relationships; change only the area, value, chroma, adjacency, illumination, or ownership that has a visible reason. Colour authorship is judged by a stronger hierarchy and more convincing material/light relationship, not by greater saturation.

## Separate Local Colour, Illumination, Environment, and Grade

Build colour in four stages:

1. **Local colour:** source-informed pigment/material families and protected identity colours.
2. **Primary illumination:** one motivated source modifies the planes it can actually reach.
3. **Environment influence:** limited bounce, reflected colour, atmosphere, and adjacency effects support depth and integration.
4. **Final grade:** a restrained unifying shift applied only after the first three stages already work.
...
When light tactile depth is used, tiny ridge catches may sit on a few compatible raised strokes. They are viewing-light accents, not new colour roles, not universal white highlights, and not permission to desaturate thick paint. Remove them mentally: the dominant field, structural counter, focal apex, protected local colours and shadow families must remain complete.
...
- global saturation or contrast that makes multiple large regions peak at once;
- a final grade that changes protected local colour, turns neutral bridges into dirty mid-chroma, or gives skin and unrelated materials the same cast;
