...
4. **Value and layered-colour contract** — define three large value groups plus spatial owners for dominant field, structural counter, focal apex, and neutral bridge or connector. Lock the source neutral evidence, `Source-neutral / Preserve source bias / Motivated shift` temperature policy, named temperature owners/boundaries, local-colour families, source-relative chroma budget, primary illumination, limited environment influence, protected colours and final-grade limits—not exact RGB values.
...
9. **Anti-filter, anti-trace, and anti-grade contract** — reject photographic shading beneath global texture; salient-prop incidentals that align as a copy; a recognizable but aesthetically degraded focal face; local colour overwritten by illumination; an unexplained whole-frame warm/cool shift; and LUT/saturation doing the work of composition or light. The source/result difference must survive thumbnail inspection and material differences must survive close inspection.
...
- subtle hue shifts that keep the same colour owner, dominance hierarchy, protected neutrals, and declared temperature policy;
...
Allowed variation becomes a failure when it changes the first read, breaks a recognition anchor, redistributes the major masses, swaps colour roles, violates the declared source-led temperature policy, equalizes focal and background contrast, or makes important materials share one surface treatment.
...
Source-led colour baseline: [reliable neutral evidence; Source-neutral / Preserve source bias / Motivated shift; named shift owner and boundary; protected neutrals; source-relative chroma budget; explicit no-automatic-warm/cool clause]
...
5. A model set is consistent only when every output preserves the contract. Report remaining variation as brush residue, micro-detail variance, colour-role drift, temperature/neutral drift, geometry drift, or another specific module.
