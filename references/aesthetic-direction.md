# Aesthetic Direction, Focal Beauty, and Subject Light
...
These two anchor sets must not be merged.
...
- scene topology or protected text explicitly requested by the user.
...
Recognition is not pixel tracing. A result fails when it preserves every incidental contour, panel, dot, fold, highlight, or colour fluctuation so literally that the transformation is only a surface treatment.

## Compose for the Finished Image

Before rendering, state one visible compositional improvement and why it benefits the subject. Choose from the actual source problem rather than applying a standard crop.
...
- merge secondary objects into broader masses so they support rather than compete;
- reshape a source light or colour region into a clearer counterform around the focal subject.

Do not restage merely to prove that change occurred. The result should retain the source's semantic identity while showing a visibly better hierarchy, rhythm, and viewing path.
...
- overall head and jaw proportion;
- eye spacing, tilt, gaze direction, and brow relationship;
- nose length/width and its relation to mouth and chin;
- mouth width, corner direction, and expression tension;
...
For a signature prop, keep a short semantic contract and redesign the visible construction.

Preserve when relevant:

- object category and functional state;
- owner, grip, contact, orientation, and relation to the body;
- signature dominant colour family;
- one or two recognition-critical motifs or geometry cues.

Author:

- silhouette rhythm and controlled proportion stylization;
- internal panels and plane breaks;
- motif grouping, spacing, scale, omission, and edge behavior;
- material response, folds, reflections, wear, and paint marks;
- overlap with the subject and surrounding negative space.

For patterned objects, preserve the pattern family rather than each repeat. A dotted umbrella remains a dotted umbrella, but its canopy planes, dot rhythm, spacing, scale, partial occlusion, highlight groups, and edge treatment should be rebuilt as a designed painted object. The same rule applies to checks, stripes, embroidery, windows, foliage, roof tiles, feathers, scales, hardware, and repeated lights.

Fail the prop transformation when the output could be aligned over the source and most incidental panel lines, pattern repeats, folds, highlights, and contour accidents would coincide.

## Build the Subject–Illumination Relationship

Treat colour and illumination as four connected but distinct layers:

1. **Local colour:** the object's own recognizable pigment or material family.
2. **Primary illumination:** one motivated source with visible direction and geometry.
3. **Environment influence:** limited bounce, reflected colour, atmosphere, and adjacency effects.
4. **Final grade:** a restrained scene-wide adjustment after local colour and light already work.

The focal subject should own the clearest useful light relationship, not necessarily the brightest pixels. Define:

- key-light direction and the planes it reveals;
- shadow side and retained internal separation;
- optional bounce or reflected colour from a nearby field;
- the background value/hue family that separates the focal silhouette;
- the brightest useful accent and deepest useful contact or occlusion on the focal path;
- the areas deliberately kept quieter so the subject remains dominant.

Let illumination influence local colour rather than replacing it. Skin, cloth, wood, metal, foliage, water, snow, and painted props must not share one highlight hue, shadow hue, or gloss response. “Flattering” means coherent facial planes, readable expression, controlled edges and motivated separation—not automatic warmth. Avoid full-rim outlining, face-shaped glow stickers, generic orange skin against blue backgrounds, a person-only beauty grade, and a uniform brightening mask over the subject.

## Grade after the Image Works

Use this order:

1. establish the source-informed local-colour families;
2. author value and colour-area relationships;
3. apply primary light and limited environmental influence;
4. assign focal versus context chroma, edge, and contrast ownership;
5. apply a restrained final grade that unifies without overwriting the first four stages.

The source palette is neither a prison nor permission to intensify every hue. Preserve successful hue relationships, source neutral/temperature character and identity colours; rebalance weak area, value, chroma, or adjacency relationships; re-script only when justified or requested. Do not globally warm or cool the frame merely to make the subject feel polished.

Final grading may shift the overall atmosphere, but it must not:

- make all greens, blues, reds, or skin tones peak simultaneously;
- turn neutral bridges into dirty mid-saturation colour;
- collapse distinct materials into the same cast;
- push highlights toward chalky white or shadows toward chromatic mud;
- use saturation as a substitute for focal hierarchy;
- erase the difference between local colour and coloured light.

## Texture and Tactile Depth Follow the Direction

Construct large connected colour and value shapes first. Marks then describe material and form; light tactile depth may finally reinforce a few overlaps, contacts, turns, crests, or foreground separators.

- Keep faces and broad skin quiet.
- Let hair follow grouped directional wedges.
- Let fabric follow fold tension and gravity.
- Let rigid props use clearer planar divisions and limited edge catches.
- Let organic fields use grouped rhythms with a few foreground accents.
- Keep atmosphere and broad continuous fields connected.

Do not use coarse paint, raised tiles, aggressive faceting, or uniform brush noise to manufacture “generation feeling.” Designed departure must come from composition, shape, light, colour, and material decisions.
...
- **Grade restraint:** the frame is coherent without dirty global saturation, clipped chroma competition, chalky highlights, or coloured mud.
- **Texture subordination:** the result still works if visible surface texture and tactile height are mentally removed.
