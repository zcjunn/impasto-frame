---
name: impasto-frame-skill
description: Create or edit original painterly frames with one thumbnail-visible directorial change, explicit focal/support/context contrast ownership, scene-owned colour, hard-locked human geometry with coherent painterly integration, redesigned salient props, subject-owned illumination, faceted material planes, and restrained tactile brush depth. Use when a photo or prompt should become a strongly stylized painted frame, or for reviewing human drift, pasted-person separation, literal prop tracing, dirty grading, filter, mosaic, patchwork, and uniform-relief failures. Do not use for faithful photo correction, pixel-locked preservation, all-over heavy impasto, sculpted wall relief, or copying a named work, character, logo, or exact frame.
---
...
- Whenever an Edit Target contains a visible person, read [references/human-preservation.md](references/human-preservation.md). Human facial geometry, body proportions, pose, limbs, hands, feet, clothing silhouette, and contact relationships are a hard lock; the person must still share the finished frame's light, colour, edge, plane, and material system rather than looking pasted over a redesigned environment.
...
- Separate recognition anchors from design anchors. Preserve identity, action, ownership/contact, semantic object state, essential topology, and signature colour families; actively author crop, negative space, environment scale, silhouette rhythm, object plane grouping, prop pattern rhythm, light envelope, colour-area balance, and final grade.
- For visible people in an Edit Target, preserve the full human geometry contract from `human-preservation.md`: facial landmarks and ratios, expression and gaze, head/neck relation, body proportions, exact visible pose, limb count/length/joints, hands, feet, clothing silhouette, and person–prop/ground contacts. Do not locally enlarge, reshape, re-pose, beautify, or reconstruct anatomy. A whole person may change apparent size only through an authorized crop/restage or a uniform undistorted whole-person transform.
- Preserve human geometry without preserving a photographic cutout. Re-express light/shadow grouping, edge hierarchy, paint handling, and material response inside the locked silhouette so the person and the transformable environment belong to one coherent finished painting. Geometry drift and pasted-person style separation are both failures.
...
2. Visible-person hard lock and declared identity/structure invariants
3. Focal aesthetic integration, semantic prop anchors, and authored design anchors
...
2. Build the Source Card from [references/source-analysis.md](references/source-analysis.md); separate `observed` from `inferred`, then choose identity fidelity, transformation mode, scene emphasis, abstraction strength, exposure key, and color-authorship mode independently. When a visible person exists, record the exact human hard lock and style-integration plan from [references/human-preservation.md](references/human-preservation.md). When another recognition-critical object/prop exists, record its recognition anchors and separately authored design anchors from [references/aesthetic-direction.md](references/aesthetic-direction.md).
3. Distill recognition anchors from transformable detail. Write the dramatic proposition, purposeful composition improvement, primary macro departure, supporting move, six-dimension three-tier contrast budget, optional color-collision decision, and thumbnail difference target from [references/directorial-contrast.md](references/directorial-contrast.md). For a visible person, direct the macro change into crop, whole-person-to-environment scale, environment forms, negative space, depth, light shape, or colour zoning without changing internal anatomy, pose, expression, or contacts; plan a shared painterly light/edge/material treatment inside the locked human geometry. For a salient prop, retain semantic anchors while redesigning silhouette, internal planes, motif rhythm and overlap. Rebuild silhouette, internal planes, and large environment shapes before specifying brush texture. For environment-emphasis, define one hero form, one counterform/current, a macro-shape budget, and source-owned transform decisions from [references/environment-abstraction.md](references/environment-abstraction.md).
4. Audit the source palette and build a role-based color plan from [references/color-authorship.md](references/color-authorship.md). Separate local colour, primary illumination, limited environment influence and final grade; preserve successful source relationships, but actively correct only the area/value/chroma/adjacency problems that weaken the focal path. Select one primary contrast axis and at most one subordinate axis, one composition family, one value plan, one subject-light hierarchy, and a different mark grammar for each important material. When tactile depth is requested, map a small set of relationship-driven depth carriers and explicitly keep broad continuous or distant passages low-relief using [references/light-depth-brushwork.md](references/light-depth-brushwork.md).
5. Compile a short priority-ordered prompt using [references/prompt-compiler.md](references/prompt-compiler.md). Before model-specific wording, write the portable render contract from [references/model-consistency.md](references/model-consistency.md): canvas and recognition/design anchors; any visible-person geometry hard lock plus style-integration plan; macro masses; value and colour-role ownership; the explicit six-dimension focal/support/context contrast budget; prop semantic anchors and redesigned construction; subject–illumination relationship; restrained grade; material grammars; edge hierarchy; optional light-depth owners and quiet fields; and anti-filter failure conditions. Explicitly reject photographic underpainting with a global paint filter when the user wants strong stylization.
...
7. Inspect the returned image beside the source at 128–256 px, blurred/thumbnail scale, mid scale, and close scale. Apply [references/quality-gate.md](references/quality-gate.md), the aesthetic gates in [references/aesthetic-direction.md](references/aesthetic-direction.md), the human geometry/style-integration gates in [references/human-preservation.md](references/human-preservation.md), and the contract-conformance test in [references/model-consistency.md](references/model-consistency.md). For visible people, compare facial landmarks, gaze/expression, head/neck, body proportions, every visible joint/limb, hands, feet, clothing silhouette, and contacts; also fail a geometrically preserved person that looks photographic, pasted, differently lit, or differently graded from the painterly environment. For Style-first/Expressive edits, fail when the macro/colour-block map remains essentially interchangeable with the source even if close-up brushwork is attractive. When comparing different models, judge whether every output passes the same contract; do not use pixel similarity, seed reuse, or an average score as proof of consistency. Fail all-over raised patchwork, tessellated knife marks, repeated equal-size paint tiles, or relief that competes with the composition and colour.
8. If a specific module fails, make at most one targeted correction. Preserve successful modules and correct only the failed human geometry, human-style integration, prop, composition, light, colour, grade, material, or tactile-depth module. Reinspect the corrected result; never trade identity or anatomy for style coherence, or style coherence for a pasted photographic person.
